...
namespace mundo_veg.Controllers
{
    public class UsuarioPjsController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,CNPJ,Nome_do_Estabelecimento,Email,Telefone,Rua,Numero,Bairro,Cidade,Estado,CEP,Horário_funcionamento,faz_entrega,tempo_entrega,descricao,Senha")] UsuarioPj usuarioPj)
        {
            if (id != usuarioPj.Id)
            {
                return NotFound();
